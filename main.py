...
# ---------------- DATA ----------------
admins = {}          # admin user_id: {channel_id, narx, karta, group_id, code}
...
@dp.message(ContentTypeFilter(content_types=types.ContentType.PHOTO))
...
if __name__ == "__main__":
    ...
    uvicorn.run(app, host="0.0.0.0", port=port)    return {"status": "Bot ishlayapti"}
...
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 10000))
    uvicorn.run(app, host="0.0.0.0", port=port)startswith("ok_"):
        try:
            link = await bot.create_chat_invite_link(chat_id=channel_id, member_limit=1)
            await bot.send_message(user_id, f"✅ Qabul qilindi!\nHavola:\n{link.invite_link}")
        except Exception as e:
